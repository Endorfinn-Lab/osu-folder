                            ...
                            if mode != "All":
                                ...
                                if mode_value is not None:  
                                    try:
                                        ...
                                        print(f"Invalid mode value in beatmap: {mode_value}") 
                            else:
                                found_in_folder = True
            if found_in_folder:
...
def delete_selected_beatmaps(key, mode):
    ...
    if messagebox.askyesno("Confirm Deletion", "Are you sure you want to delete the selected beatmaps?"):
        ...
        for index in selected_indices:
            ...
            if key:
                for filename in os.listdir(folder_path.copy()): 
                    if filename.endswith(".osu"):
...
def refresh_folder():
    ...
    if osu_folder_path:
        update_beatmap_count()
        search_beatmaps(key_entry.get(), mode_var.get()) 
...
key_label = tk.Label(window, text="Circle Size:", bg="#f0f0f0")
...
search_button = tk.Button(
    ...
    command=lambda: search_beatmaps(key_entry.get(), mode_var.get()),  
    bg="#4CAF50",
    fg="white",  
)
...
delete_button = tk.Button(
    ...
    command=lambda: delete_selected_beatmaps(key_entry.get(), mode_var.get()),  
    bg="#ff9800",  
    fg="white",
)
...
delete_all_button = tk.Button(
    ...
    command=lambda: delete_all_beatmaps(key_entry.get(), mode_var.get()), 
    bg="#ff9800",  
    fg="white",
)
